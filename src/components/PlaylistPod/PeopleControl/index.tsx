...
export default function PeopleControl({ userRecord, onUserPlayAudio }: { userRecord: UserPlayingType[]; onUserPlayAudio: Function }) {
    ...
    <Box>
      {userRecord.length ? (
        userRecord.map((user, index) => (
          <Grid container key={user.userId} alignItems='center' justifyItems={"center"} padding={1} onClick={() => onUserPlayAudio(index, true)}>
            <Grid item xs={1}>
              {user.isPlaying ? (
                playingStatus ? (
