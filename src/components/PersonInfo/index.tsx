...
export default function PersonInfo(props: PersonInfoType) {
  ...
  return (
    <Box className="flex gap-2">
      {props.isShowAvatar && <Avatar src={userInfo.avatarUrl} alt="avatar-icon" children={userInfo.nickName?.slice(0, 1)} />}
      <Box className="flex flex-col justify-center">
        {props.isShowName && <Typography className="text-base-medium">{userInfo.nickName}</Typography>}
      </Box>
    </Box>
  );
}
